...
- [x] 1.1 Update `doc/schema.md`: add `reading TEXT` to the `word_forms` DDL, change
...
- [x] 1.2 State in `doc/schema.md` that `db_info.version` is the only version authority and that
      `PRAGMA user_version` is not maintained, so neither platform may branch on it

## 2. Rust (PC) — database layer and migration

- [x] 2.1 Add `reading: Option<String>` to `WordForm` in `rust/src/db/types.rs`
- [x] 2.2 In `rust/src/db/schema.rs`: bump `CURRENT_VERSION` to 2, add `reading` to the
...
- [x] 2.3 Make `migrate()` write `db_info.version = CURRENT_VERSION` after applying steps, so
      that re-opening a migrated file runs no statement and cannot fail with `duplicate column name`
- [x] 2.4 Read and write the new column in `rust/src/db/sqlite.rs` (word form select/insert),
      trimming both value and reading and storing `NULL` for an empty reading
- [x] 2.5 Carry the reading through `rust/src/db/memory.rs` so both implementations round-trip
      identically
- [x] 2.6 Add a migration test: open a v1 fixture, assert `db_info.version` becomes 2, the column
      exists, and pre-existing rows keep their label/value with `reading = NULL`
- [x] 2.7 Add a test that opens the migrated database a second time and succeeds, proving the
      migration is not re-applied
- [x] 2.8 Add round-trip tests for form readings in the SQLite and in-memory implementations,
...
- [x] 3.1 In `rust/src/db/labels.rs`: remove `phonetic` from `EN_FORM_LABELS` and `hiragana` from
...
- [x] 3.2 Add `expected_reading` to `ConjugationField` and populate it in
      `conjugation_fields_for()` in `rust/src/quiz/engine.rs`
- [x] 3.3 Introduce the shared matching predicate — trim both sides, Unicode case folding (compare
...
- [x] 3.4 Express "unspecified" as "value and reading are both empty", which subsumes the current
...
- [x] 3.5 Grade synonym forms against the synonym form's value and reading as well
- [x] 3.6 Include the reading in the typing-mode reveal: build `word_display` for
...
- [x] 3.7 Add the reading to the word-edit state in `rust/src/ui/mod.rs`: widen
...
- [x] 3.8 Change the save filter in `rust/src/ui/mod.rs` to drop a form row only when its value
      and reading are both empty, so a reading-only row is kept
- [x] 3.9 Render the reading input per word form row in `rust/src/ui/word_edit.rs`, and keep a
      non-canonical label (such as a retired one) selectable so it survives an edit
- [x] 3.10 Show the readings on the answer side in `rust/src/ui/quiz.rs` for the base word and
      every field, whether answered correctly or not, showing a reading-only field's reading
      alone without empty parentheses
- [x] 3.11 Add engine tests: form reading accepted, form value accepted, form without a reading
...
- [x] 4.1 Add `reading: String?` to `WordForm` in `data/model/Models.kt`
- [x] 4.2 In `DbTableSQLite.kt`: bump `CURRENT_VERSION` to 2 and add `reading` to the
      `word_forms` create statement
- [x] 4.3 Drive the v1→v2 migration from `db_info.version` (for example an ensure-schema step run
...
- [x] 4.4 Keep `onUpgrade` as a safety net and make the shared migration step idempotent by
      checking whether the `reading` column already exists, so the two paths cannot both issue
      the `ALTER TABLE` in one open
- [x] 4.5 Write `db_info.version = CURRENT_VERSION` once the migration has been applied
- [x] 4.6 Read and write the new column in `DbTableSQLite.kt` word form queries, trimming both
      value and reading and storing `NULL` for an empty reading
- [x] 4.7 Carry the reading through `DbTableMemory.kt`
- [x] 4.8 Add a migration test for a normal v1 file: version becomes 2, column exists, rows intact
- [x] 4.9 Add a migration test for a file with `db_info.version = 1` but `user_version = 0`
      (as produced by the desktop app), asserting the column is added and the version written back
- [x] 4.10 Add a test that re-opens a migrated database without error, and one that exercises both
      migration paths in a single open without a duplicate-column failure
- [x] 4.11 Add form-reading round-trip tests to `DbTableSQLiteTest.kt` and `DbTableMemoryTest.kt`

## 5. Kotlin (Android) — labels, quiz engine, UI

- [x] 5.1 In `ui/Labels.kt`: remove `phonetic` and `hiragana` from the canonical lists, keep
      `kanji` and `pitch_accent`, and keep `formLabelResId()` mappings and string resources
- [x] 5.2 Give `TypingField` a reading and build the base field as
      `TypingField("word", word.word, word.reading)` in `quiz/QuizEngine.kt`
- [x] 5.3 Delete the `field.label == "word"` special case in `gradeTyping` and grade every field
      with the shared value-or-reading predicate, including synonym form lookups
- [x] 5.4 Apply the agreed comparison semantics — trim both sides, `equals(ignoreCase = true)` for
...
- [x] 5.5 Expose the correct reading alongside `correctValue` in `TypingFieldResult`
- [x] 5.6 Add a reading input per word form row in `ui/wordedit/WordEditSheet.kt` and carry it
      through `FormField` and the save mapping in `WordEditViewModel.kt`, trimming value and reading
- [x] 5.7 Change the save filter so a form row is dropped only when its value and reading are
      both empty, replacing the current label-only filter
- [x] 5.8 In `ui/quiz/QuizScreen.kt`: remove the `if (!fr.correct)` guard so every field shows its
...
- [x] 5.9 Add `QuizEngine` unit tests mirroring the Rust cases (form reading accepted, form value
...
- [x] 6.1 Confirm the Kotlin homophone dialog matches the specified behavior — same `reading`, or
...
- [x] 6.2 Leave the Rust Homophones and More info menu items as the existing stubs they are today;
      record the missing implementations as an existing gap rather than fixing them here

## 7. Verification

- [x] 7.1 Run the full Rust test suite and the Android unit tests
- [x] 7.2 Manually verify on both platforms: add a JA verb with form readings, quiz it typing the
...
- [x] 7.3 Verify a pre-change (v1) database opens, migrates to v2, keeps its words and forms, and
      opens again cleanly
- [x] 7.4 Verify the cross-platform path: copy a desktop-produced v1 file onto Android, open it,
