...
- [ ] 1.1 Rewrite `doc/schema.md` for a single version: add `words.transitivity` and
      `words.verb_group`, keep `word_forms.reading` in the base DDL, and remove the Migrations
      section describing v1→v2
- [ ] 1.2 Keep the version-policy section, including the rule that `db_info.version` is the only
...
- [ ] 2.1 In `rust/src/db/schema.rs`: set `CURRENT_VERSION` back to 1, add `transitivity` and
      `verb_group` to the `words` DDL, and keep `reading` in the `word_forms` DDL
- [ ] 2.2 Remove only the v1→v2 step from `migrate()`. **Keep** the version guard, the
...
- [ ] 2.3 Delete the v1→v2 migration tests (`v1_db_migrates_to_v2_with_rows_intact`,
      `migrated_db_reopens_without_duplicate_column_error`) and their `write_v1_db` fixture
- [ ] 2.4 Add `transitivity: Option<String>` and `verb_group: Option<String>` to `WordEntry`,
      `NewWord`, and `UpdateWord` in `rust/src/db/types.rs`
- [ ] 2.5 Read and write both columns in `rust/src/db/sqlite.rs`, storing `NULL` when absent
- [ ] 2.6 Carry both through `rust/src/db/memory.rs`
- [ ] 2.7 Add round-trip tests: a verb with both attributes, and a noun with neither

## 3. Rust (PC) — labels, locale, edit UI

- [ ] 3.1 In `rust/src/db/labels.rs`: add `transitive_pair` to `suggested_labels("ja","verb")`
      and remove the `("ja","noun")` branch entirely, so Japanese nouns suggest nothing and both
      platforms implement the same table
- [ ] 3.2 Add canonical key lists and locale-key mappings for the transitivity values
      (`intransitive` / `transitive` / `ambitransitive`) and the verb groups
      (`godan` / `ichidan` / `irregular`)
- [ ] 3.3 Add the display strings for both sets to all three locales in `rust/src/locale/mod.rs`
- [ ] 3.4 Extend the word-edit state in `rust/src/ui/mod.rs` with both attributes, add their
      messages, and clear them when the word is not a Japanese verb
- [ ] 3.5 Render the transitivity radio group and the verb group control in
      `rust/src/ui/word_edit.rs`, shown only for Japanese verbs
- [ ] 3.6 Add a test that the locale tables cover every transitivity and verb-group key

## 4. Rust (PC) — quiz engine

- [ ] 4.1 Change the empty-expectation rule in `rust/src/quiz/engine.rs`: a field of the
...
- [ ] 4.2 Add the `transitive_pair` field to Japanese verb questions via the suggestion list
- [ ] 4.3 Add the transitivity question as a distinct choice-typed field carrying the expected
      key, graded by exact match and contributing to the overall verdict
- [ ] 4.4 Expose the expected type on reveal so the UI can mark it, and make a field with no
      expectation reveal as `-`
- [ ] 4.5 Update existing tests that relied on an empty field accepting anything
- [ ] 4.6 Add engine tests: empty expectation requires an empty answer, blank is correct for a
...
- [ ] 5.1 Render the transitivity question as a radio group in `rust/src/ui/quiz.rs` with no
      option preselected, and carry the choice into grading
- [ ] 5.2 Show `-` on reveal for a field whose expected answer is empty
- [ ] 5.3 Mark the type question with ✓ / ✗ alongside the text fields

## 6. Kotlin (Android) — schema and database layer

- [ ] 6.1 In `DbTableSQLite.kt`: set `CURRENT_VERSION` back to 1, add both columns to the `words`
      DDL, and keep `reading` in the `word_forms` DDL
- [ ] 6.2 Remove only the v1→v2 step from `migrateFromDbInfo()`. **Keep** the function, its
      version-authority check, the `db_info.version` write-back, the `hasColumn` guard, and the
      `onOpen` / `onUpgrade` call sites
- [ ] 6.3 Delete the v1→v2 migration tests, including
...
- [ ] 6.4 Add `transitivity: String?` and `verbGroup: String?` to `WordEntry` in `Models.kt`
- [ ] 6.5 Read and write both columns in `DbTableSQLite.kt`, storing `NULL` when absent
- [ ] 6.6 Carry both through `DbTableMemory.kt`
- [ ] 6.7 Add round-trip tests mirroring the Rust ones

## 7. Kotlin (Android) — labels, strings, edit UI

- [ ] 7.1 Add `transitive_pair` to the Japanese verb suggestions in `WordFormLabels.forWord()`,
      and confirm Japanese nouns still return an empty list
- [ ] 7.2 Remove the fallback in `QuizEngine.buildTypingCard` that quizzes every form a word has
...
- [ ] 7.3 Add key lists and `stringResource` mappings for the transitivity and verb-group values
      in `ui/Labels.kt`
- [ ] 7.4 Add the display strings to `values`, `values-zh-rTW`, and `values-zh-rCN`
- [ ] 7.5 Extend `WordEditViewModel` with both attributes and clear them when the word is not a
      Japanese verb
- [ ] 7.6 Render the transitivity radio group and the verb group control in `WordEditSheet.kt`,
      shown only for Japanese verbs

## 8. Kotlin (Android) — quiz engine and UI

- [ ] 8.1 Restructure `gradeTyping` in `quiz/QuizEngine.kt` to first decide **which word** is
...
- [ ] 8.2 Add the `transitive_pair` field and the transitivity question to Japanese verb cards,
      both counting toward `allCorrect`
- [ ] 8.3 Expose the expected type in `TypingFieldResult` for the reveal
- [ ] 8.4 Render the type question as radio buttons in `QuizScreen.kt` with none preselected,
      and show `-` for a field with no expected answer
- [ ] 8.5 Update every existing test invalidated by either the new grading rule or the changed
...
- [ ] 8.6 Add `QuizEngine` tests mirroring the Rust cases

## 9. Seed regeneration

- [ ] 9.1 Rewrite the generator to read the Markdown sources and emit SQL for the new schema,
...
- [ ] 9.2 Derive `verb_group` from the dictionary and ない forms rather than the source's
...
- [ ] 9.3 Classify transitivity for every verb: carry over the 98 explicit markings, infer from
      partners and morphology where possible, and judge the remaining ~104
- [ ] 9.4 Emit a consistency report covering: pairs whose two sides disagree on type, partners
...
- [ ] 9.5 Review the report and the classifications: correct the 4 pairs whose sides disagree,
...
- [ ] 9.6 Derive the adjective forms mechanically and emit them, so i-adjectives and
      na-adjectives stop quizzing with empty fields: い → くて / くない / かった,
      な → で / じゃない / だった, with `いい` irregular (よくて / よくない / よかった)
- [ ] 9.7 Stop writing `動詞類` and `自他` into `words.note` for verbs, keeping `note` for the
      noun 補充 column

## 10. Verification

- [ ] 10.1 Run the full Rust test suite and the Android unit tests
- [ ] 10.2 Delete every existing database **before installing the new build on Android**:
...
- [ ] 10.3 Apply the regenerated seed with the desktop app closed
...
- [ ] 10.4 Manually verify on both platforms: a verb with a partner accepts the partner and
...
- [ ] 10.5 Confirm neither Japanese-verb question appears for English words or Japanese nouns
